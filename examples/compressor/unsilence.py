...
D = [x for x in os.listdir('mxaudio') if (not ('z_' in x)) and ('o_x_' in x) & ('.wav' in x) & ('MX20' in x)]
print(D)
for FI in D:
    ...
    for FI2 in [FI, FI.replace('_x_','_y_')]:
        ...
        subprocess.call(f'aws s3 cp {OPATH} s3://meeshkan-datasets/mxaudio-unsilenced/{FI2}', shell=True)
        os.remove(OPATH)